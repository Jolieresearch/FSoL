...
class FakeTT_Dataset(Base_Dataset):
    ...
    def get_data(self, split: str = None) -> pd.DataFrame:
        ...
        try:
            ...
            logger.info(f"Loaded FakeTT dataset{split_info} with {len(data)} samples")
            
            return data
            
        except Exception as e:
            logger.error(f"Error loading FakeTT dataset: {e}")
            return pd.DataFrame()
    ...
    def validate_split_consistency(self):
        """
        验证split文件中的ID是否都在frames目录中存在
        """        
        split_files = {
            'train': 'data/FakeTT/vids/vid_time3_train.txt',
            'valid': 'data/FakeTT/vids/vid_time3_valid.txt', 
            'test': 'data/FakeTT/vids/vid_time3_test.txt'
        }
        

        available_frames = set()
        if self.frames_path.exists():
            available_frames = set(d.name for d in self.frames_path.iterdir() if d.is_dir())
        ...
        for split_name, split_file in split_files.items():
            if os.path.exists(split_file):
                try:
                    ...
                    missing_frames = split_ids - available_frames
                    
                    validation_results[split_name] = {
                        'total_ids': len(split_ids),
                        'missing_frames': list(missing_frames),
                        'missing_count': len(missing_frames),
                        'available_count': len(split_ids - missing_frames),
                        'valid': len(missing_frames) == 0
                    }
